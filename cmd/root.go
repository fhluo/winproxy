...
func init() {
	...
	rootCmd.Flags().BoolVar(&direct, "direct", settings.Direct(), "")
	rootCmd.Flags().BoolVar(&useProxy, "use-proxy", settings.UseProxy(), "")
	rootCmd.Flags().BoolVar(&useScript, "use-script", settings.UseScript(), "")
	rootCmd.Flags().BoolVar(&autoDetect, "auto-detect", settings.AutoDetect(), "")

	rootCmd.Flags().StringVar(&settings.ProxyAddress, "proxy-address", settings.ProxyAddress, "")
	rootCmd.Flags().StringVar(&settings.BypassList, "bypass-list", settings.BypassList, "")
